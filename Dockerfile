...
RUN native-image -cp ".:jar_files/*" --shared -H:Name=libbfbridge  --initialize-at-run-time=sun.awt --initialize-at-run-time=javax.imageio --initialize-at-run-time=java.awt --initialize-at-run-time=com.sun.imageio --initialize-at-run-time=sun.java2d --initialize-at-run-time=sun.font --initialize-at-run-time=loci.formats.gui.AWTImageTools --initialize-at-run-time=ome.specification.XMLMockObjects --initialize-at-run-time=ome.codecs.gui.AWTImageTools --initialize-at-build-time --enable-preview org.camicroscope.BFBridge
RUN ls
